...
- **Project access and collaborators:** access matches the assignment wording—**own the project or have tasks on it**—and the same rule gates nested task routes. That keeps **`GET /projects`** consistent with who may **`POST …/tasks`**, but it means a **non-owner usually needs an existing task** (e.g. as **assignee**, created by the owner) before they can add more work. With more time, add **memberships / invites**, or explicit roles (**who may create tasks**), if the product should onboard people without that first-task link.
